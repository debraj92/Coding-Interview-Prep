...
public class PracticeComparator {
	public static void main(String[] args) {
		 ...
		 * Comparable interface. But we still want to sort them based on some logic. Or
		 * we want to decide at different situations on what basis we want to sort our
		 * objects.
		 * 
		 * Comparator is a separate utility passed on the the sorting method which can
		 * do the comparison that the sorting method will need. In case of comparable
		 * the objects define themselves as comparable and so they have to compareTo
		 * method. For comparator the objects themselves don't know if they can be
		 * compared. The comparator is an external utility that has some logic to
		 * compare objects which are otherwise not comparable
		 */
		MobileBrand[] brands = new MobileBrand[] { new MobileBrand("Lenovo", 10), new MobileBrand("Samsung", 20),
				new MobileBrand("Apple", 15), new MobileBrand("LG", 8), new MobileBrand("Motorola", 7),
				new MobileBrand("OPPO", 5), new MobileBrand("Huawei", 13) };
...
class BubbleSortUtil {
	public void sort(MobileBrand[] allBrands, Comparator<MobileBrand> comparator) {
		/**
		...
		for (int i = 0; i < allBrands.length - 1; i++) {
			for (int j = 0; j < allBrands.length - i - 1; j++) {
				if (comparator.compare(allBrands[j], allBrands[j + 1]) > 0) {
					// jth object greater than j+1 th object. Swap them
					MobileBrand temp = allBrands[j];
					allBrands[j] = allBrands[j + 1];
					allBrands[j + 1] = temp;
				}
			}
		}
